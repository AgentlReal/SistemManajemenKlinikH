...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::statement("CREATE VIEW `view_karyawan_tanpa_jadwal` AS select 'Dokter' AS `jenis_karyawan`,`d`.`id_dokter` AS `id_karyawan`,`d`.`nama` AS `nama_karyawan`,`d`.`spesialis` AS `spesialis`,`p`.`nama_poli` AS `unit_kerja`,`d`.`nomor_telepon` AS `nomor_telepon`,`d`.`jenis_kelamin` AS `jenis_kelamin`,NULL AS `jabatan_lainnya` from (`simakapi`.`dokter` `d` join `simakapi`.`poli` `p` on((`d`.`id_poli` = `p`.`id_poli`))) where `d`.`id_dokter` not in (select distinct `simakapi`.`jadwal_karyawan`.`id_dokter` from `simakapi`.`jadwal_karyawan` where (`simakapi`.`jadwal_karyawan`.`id_dokter` is not null and `simakapi`.`jadwal_karyawan`.`deleted_at` is null)) AND `d`.`deleted_at` IS NULL union all select 'Resepsionis' AS `jenis_karyawan`,`r`.`id_resepsionis` AS `id_karyawan`,`r`.`nama` AS `nama_karyawan`,NULL AS `spesialis`,'Front Office' AS `unit_kerja`,`r`.`nomor_telepon` AS `nomor_telepon`,`r`.`jenis_kelamin` AS `jenis_kelamin`,NULL AS `jabatan_lainnya` from `simakapi`.`resepsionis` `r` where `r`.`id_resepsionis` not in (select distinct `simakapi`.`jadwal_karyawan`.`id_resepsionis` from `simakapi`.`jadwal_karyawan` where (`simakapi`.`jadwal_karyawan`.`id_resepsionis` is not null and `simakapi`.`jadwal_karyawan`.`deleted_at` is null)) AND `r`.`deleted_at` IS NULL union all select 'Staf Lab' AS `jenis_karyawan`,`sl`.`id_staf_lab` AS `id_karyawan`,`sl`.`nama` AS `nama_karyawan`,NULL AS `spesialis`,'Laboratorium' AS `unit_kerja`,`sl`.`nomor_telepon` AS `nomor_telepon`,`sl`.`jenis_kelamin` AS `jenis_kelamin`,`sl`.`nomor_lisensi` AS `jabatan_lainnya` from `simakapi`.`staf_lab` `sl` where `sl`.`id_staf_lab` not in (select distinct `simakapi`.`jadwal_karyawan`.`id_staf_lab` from `simakapi`.`jadwal_karyawan` where (`simakapi`.`jadwal_karyawan`.`id_staf_lab` is not null and `simakapi`.`jadwal_karyawan`.`deleted_at` is null)) union all select 'Kasir' AS `jenis_karyawan`,`k`.`id_kasir` AS `id_karyawan`,`k`.`nama` AS `nama_karyawan`,NULL AS `spesialis`,'Administrasi' AS `unit_kerja`,`k`.`nomor_telepon` AS `nomor_telepon`,`k`.`jenis_kelamin` AS `jenis_kelamin`,NULL AS `jabatan_lainnya` from `simakapi`.`kasir` `k` where `k`.`id_kasir` not in (select distinct `simakapi`.`jadwal_karyawan`.`id_kasir` from `simakapi`.`jadwal_karyawan` where (`simakapi`.`jadwal_karyawan`.`id_kasir` is not null and `simakapi`.`jadwal_karyawan`.`deleted_at` is null))");
    }
